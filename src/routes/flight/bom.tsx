...
const fetchFlightData = async (params: URLSearchParams, signal?: AbortSignal) => {
    try {
        const [locationsRes, flightAllRes, filteredFlightsRes] = await Promise.all([
            fetch(`http://localhost:3017/locations`, { signal }),
            fetch(`http://localhost:3017/flightall`, { signal }),
            fetch(`http://localhost:3017/flights?${params.toString()}`, { signal }),
        ]);
        ...
        if (params.get("trip_type") === "roundtrip" && params.get("return_date")) {
            ...
            const returnFlightsRes = await fetch(`http://localhost:3017/flights?${returnParams.toString()}`, { signal });
...
export default function FlightSelection() {
    ...
    const [selectedTab, setSelectedTab] = useState(tabTypeParam === "helicopter" ? "helicopter" : "plane");
    ...
    useEffect(() => {
        if (!returnDateParam || !allReturnDates.length || selectedTabTrip !== "roundtrip" || selectedTab !== "helicopter") return;
    ...
    }, [returnDateParam, allReturnDates, selectedTabTrip, selectedTab, visibleCount]);
