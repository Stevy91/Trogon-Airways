...
export default function DateCarousel({
    ...
    const handleDateClick = (globalIndex: number, formattedDate: string) => {
        ...
        // Toujours conserver les paramètres existants
        params.append("from", currentSearchParams.get("from") || from);
        params.append("to", currentSearchParams.get("to") || to);
        params.append("tab", currentSearchParams.get("tab") || flightType); // Ajout du paramètre tab
                ...
                return (
                    ...
                    >
                        <div
                            ...
                            onClick={() => handleDateClick(globalIndex, formattedDate)}
                            className={`flex min-w-[80px] flex-col items-center whitespace-nowrap rounded px-3 py-2 text-sm transition-all ${
                                isSelected
                                    ...
                                    : d.hasFlight
                                      ...
                                      : "cursor-not-allowed font-bold text-red-300"
                            }`}
                            style={{ scrollSnapAlign: "center" }}
                        >
                            <span>{format(d.date, "EEE MMM d")}</span>
                            <span className={isAvailable ? "" : "p-0 text-red-300"}>
                                {isAvailable ? (
                                    `$${d.price}`
                                ) : (
                                    <NoFlightIcon
