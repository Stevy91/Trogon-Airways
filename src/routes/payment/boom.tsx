...
const API_CONFIG = {
    baseUrl: "http://localhost:3017", // Remplacez par votre URL backend
    apiKey: "pk_test_51IxWQcCWPjcmZuUZHO2WQWN23llRV6peeOK563IjPU8mONaLs8jmkFniUASfkWZ530ROaRj9VQUTfFwCRbTeP0Ke00N8tXbld3", // À sécuriser en production
};
...
const StripePaymentForm: React.FC<StripePaymentFormProps> = ({ totalPrice, onSuccess, paymentData }) => {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            ...
            const response = await fetch("http://localhost:3017/confirm-booking", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                                ...
                                {paymentMethod === "stripe" ? (
                                    <Elements stripe={stripePromise}>
                                        <StripePaymentForm
                                            ...
                                            paymentData={paymentData} // Passez paymentData comme prop
                                        />
                                    </Elements>
                                ) : (
